...
export class ProductionDatabase implements DatabaseRepository {
  ...
  async getInvestigation(id: string): Promise<InvestigationRecord | null> {
    await this.init()
    const { db } = getDb()
  ...
  async searchInvestigations(query: SearchQuery): Promise<InvestigationListItem[]> {
    await this.init()
    const { db } = getDb()
    const q = (query.q ?? "").trim()
    const base = db.select().from(investigations)
    const rows = q
      ? await base
          .where(
            or(
          ...
          .orderBy(desc(investigations.updatedAt))
      : await base.orderBy(desc(investigations.updatedAt))
